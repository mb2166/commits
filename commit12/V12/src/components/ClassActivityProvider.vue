...
export default {
  ...
  mounted() {
    this.filterOn.push("provider"); //assign specific field to the filer
  ...
  methods: {
    ...
    topicratings(item, button) {
      ...
      for (cntr = 0; cntr < this.ClassActivityRatings.length; cntr++) {
        ...
        if (topic_index == this.ClassActivityRatings[cntr].topic_id) {
          ...
          sumratings =
            sumratings + parseInt(this.ClassActivityRatings[cntr].rating);
        }
      }
    ...
    addClassActivity() {
      ...
      this.ClassActivity.push({
        ...
        time: this.time,
        length: this.length,
        provider: this.email.toLowerCase().trim(),
        type: this.type
      });
    ...
    editClassActivity(x) {
      ...
      this.provider = this.email.toLowerCase().trim();
